// ============================================================
// EcoCharge Ghana — App.jsx
// Font Awesome icons + Inter font — All features working
...
const CSS = `
  ...
  @import url('https://cdn.jsdelivr.net/npm/@fortawesome/fontawesome-free@6.5.0/css/all.min.css');
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
  ...
  input,button,textarea,select{font-family:'Inter',sans-serif;outline:none}
  input{color:#fff}
...
const Badge = ({ label, color=T.green }) => (
  <span style={{
    ...
    borderRadius:6, padding:"3px 8px", border:`1px solid ${color}44`,
...
const NewNav = ({ active, go }) => (
  <div style={{
    ...
    padding:"10px 0 26px", borderTop:`1px solid ${T.border}`,
    background:"rgba(10,13,16,.97)", backdropFilter:"blur(16px)", zIndex:100,
  }}>
    {[
      ...
      <div style={{ padding:"52px 20px 20px", background:"linear-gradient(135deg,#0a1f12,#0f2b18)" }}>
        ...
        {user
          ? <div style={{ background:"rgba(74,222,128,.08)", borderRadius:10, padding:"10px 14px" }}>
              ...
              <div style={{ fontWeight:600, color:T.text, fontSize:13, marginTop:2, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>
                {user.email}
              </div>
            </div>
      ...
      {user && (
        <div style={{ padding:"16px 20px", borderTop:`1px solid ${T.border}` }}>
          <button onClick={()=>{ onLogout(); onClose(); }} className="tap"
            style={{ width:"100%", background:"rgba(248,113,113,.08)", border:"1px solid rgba(248,113,113,.2)",
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", height:"100%", background:T.bg,
      ...
      <div className="fade" style={{ textAlign:"center", marginBottom:40 }}>
        ...
        <div style={{ fontSize:13, color:T.muted, marginTop:12, lineHeight:1.8 }}>
          ...
          <div style={{ display:"flex", gap:10, overflowX:"auto", paddingBottom:2 }}>
            {stations.map(s=>(
              <div key={s.id} className="tap" onClick={()=>{ setStation(s); go("detail"); }}
                style={{ background:"rgba(19,23,31,.95)", backdropFilter:"blur(8px)",
                  borderRadius:14, padding:"10px 14px", border:`1px solid ${T.border}`,
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", height:"100%", background:T.bg, overflowY:"auto" }}>
      ...
      <div className="fade" style={{ margin:"0 14px 16px", borderRadius:22, overflow:"hidden",
        ...
        background:"linear-gradient(135deg,#071a09 0%,#0a2510 60%,#061f0c 100%)",
        border:`1px solid rgba(74,222,128,0.15)` }}>
        {/* Right decorative */}
        <div style={{ position:"absolute", right:0, top:0, bottom:0, width:"50%", overflow:"hidden" }}>
          <div style={{ position:"absolute", inset:0,
            background:"radial-gradient(ellipse at 70% 50%,rgba(74,222,128,0.06) 0%,transparent 70%)" }}/>
          <div style={{ position:"absolute", top:16, right:-4, left:4, height:14,
            background:"rgba(74,222,128,0.06)", borderBottom:"1px solid rgba(74,222,128,0.15)", borderRadius:"4px 0 0 0" }}/>
          <div style={{ position:"absolute", bottom:0, right:12, display:"flex", gap:10, alignItems:"flex-end" }}>
            {[52,66,48].map((h,i)=>(
              <div key={i} style={{ width:12, height:h, background:"rgba(74,222,128,0.10)",
                borderRadius:"4px 4px 0 0", border:"1px solid rgba(74,222,128,0.18)", position:"relative" }}>
                <div style={{ position:"absolute", top:8, left:"50%", transform:"translateX(-50%)",
                  width:4, height:4, borderRadius:"50%", background:T.green, boxShadow:`0 0 5px ${T.green}` }}/>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="fade3" style={{ margin:"0 14px 14px" }}>
        ...
        {(search ? filtered : stations).slice(0,3).map((s, idx)=>{
          ...
          const bgs = [
            "linear-gradient(135deg,#0a2010,#0d3018)",
          ...
          return (
            <div key={s.id} className="tap row" onClick={()=>{ setStation(s); go("detail"); }}
              ...
              <div style={{ width:80, height:80, borderRadius:12, flexShrink:0,
                background:bgs[idx%3], display:"flex", alignItems:"center",
              ...
              <div style={{ flexShrink:0, display:"flex", flexDirection:"column", alignItems:"flex-end", gap:8 }}>
                <div style={{ background:"rgba(74,222,128,0.10)", border:`1px solid rgba(74,222,128,0.22)`,
                ...
                <button onClick={e=>{ e.stopPropagation(); setStation(s); go("detail"); }} className="tap"
                  style={{ width:32, height:32, background:`linear-gradient(135deg,${T.green},${T.greenDark})`,
      ...
      {/* WATER BANNER */}
      <div style={{ margin:"0 14px 110px", background:"linear-gradient(135deg,#061520,#09202e)",
...
function Vehicles({ go, setVehicle, user }) {
  ...
  const vehicleImages = {
    Car:      "/car-charging.jpg",
    Scooter:  "/scooter-charging.jpg",
    Tricycle: "/tricycle-charging.jpg",
  };
    ...
    <div style={{ display:"flex", flexDirection:"column", height:"100%", background:T.bg }}>
      ...
      <div style={{ flex:1, overflowY:"auto", padding:"14px 14px 0" }}>
        {VEHICLES.map((v,i)=>(
          <div key={v.type} className={`tap fade${i}`} onClick={()=>setSel(v)}
            ...
            <div style={{ height:180, position:"relative", overflow:"hidden" }}>
              <img src={vehicleImages[v.type]} alt={v.type}
        ...
        <div style={{ background:T.card, borderRadius:14, padding:"14px 16px", border:`1px solid ${T.border}`, marginBottom:16 }}>
          <div style={{ fontSize:12, color:T.muted, marginBottom:8, fontWeight:600, textTransform:"uppercase", letterSpacing:0.5 }}>
  ...
  const inp = (ph, val, set, type="text", icon="fa-user") => (
    <div style={{ position:"relative", marginBottom:10 }}>
      ...
      <input type={type} placeholder={ph} value={val}
        onChange={e=>{ set(e.target.value); setErr(""); }}
        style={{ width:"100%", background:"#0c0f18", border:`1px solid ${T.border}`,
          borderRadius:10, padding:"12px 14px 12px 40px", color:T.text, fontSize:14 }}/>
    </div>
  );
      ...
      <div style={{ flex:1, overflowY:"auto", padding:"14px 14px 120px" }}>
        ...
        <div className="fade3" style={{ background:T.card, borderRadius:16, padding:"14px 16px", marginBottom:14, border:`1px solid ${T.border}` }}>
          ...
          {[
            ...
            { id:"arrive", label:"Pay on arrival",     sub:"Reserve now, pay at station",  icon:"fa-store" },
          ].map(m=>(
        ...
        <button onClick={book} disabled={loading} className="tap"
          style={{ width:"100%", background:`linear-gradient(135deg,${T.green},${T.greenDark})`,
            ...
            color:"#000", cursor:loading?"default":"pointer", fontFamily:"inherit",
            display:"flex", alignItems:"center", justifyContent:"center", gap:10,
            opacity:loading?.7:1 }}>
          {loading
            ? <><Spinner/> Processing…</>
        ...
        <div style={{ background:T.card, borderRadius:16, padding:"16px", marginBottom:12, border:`1px solid ${T.border}` }}>
          ...
          <div style={{ position:"relative", marginBottom:12 }}>
            ...
            <input placeholder="e.g. ECO-ABC123" value={code}
              ...
              style={{ width:"100%", background:"#0c0f18", border:`1px solid ${T.border}`,
                borderRadius:10, padding:"14px 14px 14px 40px", color:T.text, fontSize:16,
        ...
        {user ? (
          <>
            ...
            <button onClick={()=>setUser(null)} className="tap"
              style={{ width:"100%", background:"rgba(248,113,113,.07)", border:"1px solid rgba(248,113,113,.2)",
...
export default function App() {
  ...
  const goSecure = (s) => {
    const open = ["splash","auth","about","home","detail","verify","map"];
