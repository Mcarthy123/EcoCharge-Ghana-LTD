...
export default function App() {
  ...
  const saveToHistory = async (station, vehicle, amount) => {
  ...
  if (screen === "login" || screen === "signup") {
    return (
      <div style={{ background: bg, minHeight: "100vh", color: "#fff", fontFamily: "sans-serif", padding: 24, display: "flex", flexDirection: "column", justifyContent: "center" }}>
        <h1 style={{ color: teal, textAlign: "center", marginBottom: 4 }}>⚡ EcoChargeCar</h1>
        ...
        <div style={{ padding: "0 20px" }}>
          ...
          {["Car", "Scooter", "Tricycle"].map((v) => (
            <div key={v} onClick={() => setVehicle(v)}
              style={{ background: vehicle === v ? "#0d2018" : card, border: `1.5px solid ${vehicle === v ? teal : "#1e2e24"}`, borderRadius: 14, padding: "14px 16px", marginBottom: 10, cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                <span style={{ fontSize: 28 }}>{icons[v]}</span>
        ...
        <div style={{ padding: "0 20px" }}>
          ...
          {history.length === 0 ? (
            <div style={{ background: card, borderRadius: 16, padding: 24, textAlign: "center", border: "1px solid #1e2e24" }}>
              ...
              <p style={{ color: "#555", fontSize: 13 }}>Your charging history will appear here</p>
            </div>
          ) : (
            history.map((h, i) => (
              <div key={i} style={{ background: card, border: "1px solid #1e2e24", borderRadius: 16, padding: 16, marginBottom: 12 }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between" }}>
                  <span style={{ color: "#7a9a85", fontSize: 13 }}>{icons[h.vehicle]} {h.vehicle}</span>
